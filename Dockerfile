...
RUN apt-get install zip unzip sshpass libzip-dev libonig-dev xvfb libxi6 -y
RUN apt-get install libgconf-2-4 telnet python-pip gcc g++ make librabbitmq-dev libbz2-dev -y
RUN apt-get install libicu-dev libxml2-dev libxslt1-dev libfreetype6-dev libjpeg62-turbo-dev -y
RUN apt-get install libpng-dev git vim openssh-server ocaml expect curl libssl-dev libcurl4-openssl-dev pkg-config -y
...
RUN apt-get install net-tools openssh-server nano vim mariadb-client -y && apt-get install -y apache2 \
    ...
    && a2enmod proxy_fcgi\
    && a2enmod ssl
RUN curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer -v2.2
